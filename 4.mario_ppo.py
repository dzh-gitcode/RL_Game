...
class PPOSolver:
    def __init__(self):
        ...
        self.n_mini_batch = 4
        self.epochs = 30
        self.save_directory = "./weights/ppo"
    ...
    def load_checkpoint(self, filename):
        self.policy.load_state_dict(torch.load(os.path.join(self.save_directory, filename), map_location=device))
        self.policy_old.load_state_dict(torch.load(os.path.join(self.save_directory, filename), map_location=device))
    ...
    def sample(self):
        ...
        for t in range(self.worker_steps):
            ...
            self.obs, rewards[t], done[t], info = env.step(actions[t])
...
checkpoint_file = "checkpoint_3490.pth"
solver = PPOSolver()
